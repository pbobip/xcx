...
| 动作 | 身份 | 输入 | 输出 | 幂等 |
|---|---|---|---|---|
| `quote` | 顾客 | 套餐 ID、数量、可选顾客券 ID | 套餐、数量、云端单价、原价、券模板快照、优惠和应付金额 | 只读，不需要 |
| `create` | 顾客 | 套餐 ID、数量、动态字段值、可选顾客券 ID | 服务订单、付款状态、是否复用原订单 | `idempotencyKey` 必填 |

`quote` 与 `create` 都重新读取当前套餐和顾客券，不接受前端单价、优惠金额或应付金额。当前基础价格使用 `priceCents × quantity`；选择优惠券后由服务端校验归属、状态、有效期、门槛及游戏/专区/套餐范围，并返回最终报价。确认订单页按套餐 `orderFields` 决定现有平台、区服、游戏 ID、服务时间、预约、备注和成年确认字段是否必填；`create` 再按同一配置校验必填、选项、预约、成年确认和敏感输入并保存不可变订单快照。新订单初始为支付 `UNPAID`、履约 `NOT_STARTED`、售后 `NONE`。
...
`coupon.mine.list` 的状态为 `unused`、`used` 或 `expired`；顾客只能读取自己的券。尚未写回状态、但有效期已经结束的 `AVAILABLE` 券在读取时按 `EXPIRED` 返回；尚未生效的券仍归入 `unused`，但返回 `available = false` 和“优惠券尚未生效”。展示有效期取模板与顾客券有效期的交集。`coupon.available.list` 返回当前顾客尚未使用的券，并逐张给出 `available`、`unavailableReason`、优惠金额和券后应付金额。

`FIXED` 为无门槛立减券，`THRESHOLD` 为满减券；模板的游戏、专区和套餐范围均为空时适用于全部服务套餐。一张服务订单最多选择一张券。

锁券、核销、退回不是顾客可直接调用的公开动作。`order.create` 在建单事务内将券从 `AVAILABLE` 锁定为 `LOCKED`，同一幂等请求重试返回原订单；未付款订单关闭时在同一事务内退回 `AVAILABLE`，若券已过期则转为 `EXPIRED`，模板已停用或缺失则转为 `VOID`。支付成功后的 `LOCKED → USED` 由支付通知事务完成。
...
| `reconcile.daily` | 财务/系统 | T 日账单日期 | T+1 对账结果与差异 | 同一账单日期覆盖更新 |

`maintenance.run` 是内部定时维护动作，不属于小程序或后台公开接口。它仅接受
CloudBase 提供且 `SOURCE` 包含 `wx_trigger` 的定时触发上下文，负责批量处理
超时 `PREPAY`/`NOTPAY` 支付记录和上一自然日对账；普通客户端传入同名动作返回 `FORBIDDEN`。
...
- `order.create`：`userId + idempotencyKey` 唯一；套餐、数量、动态字段和顾客券均相同时返回原订单，任一项不同返回冲突；
