# 同类游戏服务小程序 PRD（v2.1 实施确认版）

| 项目 | 内容 |
|---|---|
| 文档版本 | v2.1 |
| 文档状态 | 实施确认基线；待云东补齐运营参数后进入后台与接口开发 |
| 编制日期 | 2026-07-27 |
| 参考原型 | 微信小程序“云玩电竞-陪玩开黑” |
| 观察方式 | 已逐页查看参考小程序，并在微信开发者工具中验收当前原生小程序 |
| 目标产品 | 与参考原型结构、体验和视觉气质相近的自营游戏服务小程序 |
| 商业模式 | 单商户自营、后台人工派单 |
| 首版终端 | 微信小程序顾客端 + Web 运营后台 |
| 当前前端基线 | `D:\脚本\xcx\miniprogram-v2` 原生微信小程序，17 个页面 |
| 技术基线 | 顾客端继续使用现有原生小程序；服务端与 Web 后台采用模块化单体，开源底座另行专项选型 |
| 当前阶段 | 阶段 A 高保真原型已完成第一轮，尚未接入真实后台、登录会话和微信支付 |
...
|---|---|---|
| 首页 | 顶部大横幅、最新服务、更多入口、推荐频道、横向专区标签、纵向服务卡片、悬浮微信客服 | 保留相同信息密度和浏览顺序；客服改为顶部工具栏入口，避免遮挡筛选和商品内容 |
...
| 消息 | 系统通知、未读角标、空状态、悬浮客服 | 首版仅做系统消息；客服改为顶部工具栏入口，不自建顾客与服务人员私聊 |
...
1. 顾客端不再改用 ShopXO UniApp 重写，继续迭代现有原生微信小程序；服务端与后台单独选型。
2. 底部 Tab 顺序固定为“首页、分类、我的、消息”，消息角标位于第 4 项。
3. 首页、分类、我的、消息的人工客服入口放在顶部工具栏；服务详情保留底部“联系客服 + 选择套餐”。
4. 视觉基线采用当前紫色电竞背景、粉色强调色、白色纸张卡片和深色描边，不再按旧版蓝青橙令牌重做。
5. 六个游戏均保留分类入口；试运营首发暂定为无畏契约，英雄联盟、永劫无间、三角洲行动、王者荣耀、绝地求生在套餐和价格确认前显示“敬请期待”。
6. 当前无畏契约价格与订单均为原型演示数据，不作为正式销售价格；正式价格由后台配置并由服务端计算。
7. 当前微信登录、支付、订单、退款和派单仍是前端演示流程；上线前必须接入服务端会话、微信支付 APIv3 回调验签和真实订单状态机。
...
一个采用紫色电竞视觉、按游戏专区组织服务套餐、支持微信支付和人工派单的自营游戏陪玩/护航小程序。
...
实施顺序：试运营首发暂定无畏契约；其余五个游戏先展示分类入口和“敬请期待”状态，待运营补齐套餐、价格、交付标准和人员能力后逐项开放。
...
- 顾客可免登录浏览；选择套餐进入确认订单前完成微信登录，并补充履约必需信息；
- 一张订单只购买一个服务套餐；
...
顾客从首页或分类进入服务详情，查看服务团队说明、支持平台、交付标准、评分和评价；点击“选择套餐”时完成微信登录，然后进入确认订单，选择数量、优惠券、游戏平台/区服，填写游戏 ID，完成成年确认并微信支付。支付成功后，运营人员在后台人工选择内部服务人员，依次更新待服务、进行中和已完成。顾客可从订单、消息和客服入口追踪进度并提出投诉或售后。
...
- 风格：深紫电竞背景、粉色霓虹强调、白色纸张卡片、深色粗描边；
- 页面背景：深紫色并带低对比点阵纹理；
- 主要卡片：近白色纸张，用深色文字保证信息可读性；
- 次级区域：紫色面板，用白色和浅粉文字；
- 主强调色与价格色：浅粉色，用于选中状态、价格、按钮和重点信息；
- 卡片使用大圆角、深色描边和硬边阴影；不使用会降低文字可读性的复杂玻璃拟态；
- 原型中的字母封面仅作为占位，正式游戏图片必须使用原创或已授权素材。
...
以上令牌以当前原生小程序为实施基线；后续只做可读性和适配优化，不整体切换回旧版蓝青橙主题。
...
底部导航固定在安全区上方，顺序固定为“首页、分类、我的、消息”；当前项使用浅粉色 `#F3C0EF` 高亮，消息角标固定在第 4 项。
...
- 在右侧窄栏中采用纵向卡片：上方横向封面、下方商品信息，避免图片挤压文字；
- 标题最多 2 行；
- 专区小图标和专区名称；
- 一行服务团队摘要；
- 原价灰色删除线；
- 现价使用粉色突出；
- 暂未开放的游戏只显示一组“套餐价格与规则配置中 / 敬请期待”，点击时给出明确提示，不重复堆叠“待配置、未开放”等文案；
- 售罄或暂停接单时显示禁用状态并禁止下单。
...
- 首页、分类、我的、消息页面使用顶部工具栏人工客服入口，禁止悬浮遮挡筛选、商品卡或底部导航；
- 服务详情使用底部固定“联系客服 + 选择套餐”双按钮，普通态和点击态均不得重叠；
...
8. 作为顾客，我希望通过页面顶部客服入口随时联系微信客服，并且入口不遮挡内容，以便迅速咨询。
...
23. 作为顾客，我希望详情页底部始终显示客服和选择套餐，以便快速行动。
...
1. 顾客端继续采用 `D:\脚本\xcx\miniprogram-v2` 原生微信小程序，不因后台底座选型重写现有 17 个页面。
2. 使用模块化单体，不为首版引入微服务、多租户或复杂消息集群。
3. 小程序前端按 750rpx 设计，优先适配微信主流屏幕与底部安全区；服务详情双按钮、确认订单支付栏和 TabBar 必须保留防重叠约束。
...
15. 服务端与 Web 后台使用模块化单体；ShopXO、CRMEB 或自建后台只能作为候选方案，须经过接口适配成本、许可证和服务订单改造量评审后确定。
...
1. 首页包含横幅、最新服务、更多、推荐专区、商品列表和顶部客服入口；客服不得遮挡专区标签或商品卡。
...
1. 整体为深紫背景 + 紫色面板 + 白色纸张卡片 + 粉色强调和价格的电竞主题。
2. 主流微信屏幕无横向溢出、底部遮挡或按钮超出安全区。
3. 服务卡标题、图片、价格和标签层级与参考产品接近。
4. 加载、空状态、网络失败、支付取消和无优惠券均有明确反馈。
5. 不使用参考产品受版权保护的原始素材。
6. 服务详情底部“联系客服/选择套餐”普通态和点击态均不重叠。
7. 确认订单可滚动至最后一个字段，固定支付栏不遮挡成年确认或备注。
8. 分类页左侧游戏菜单和右侧服务卡无横向溢出，标题、标签、说明和价格不互相覆盖。
...
- 自动化测试文件：`D:\脚本\xcx\miniprogram-v2\tests\ui-interactions.test.js`；
- 当前覆盖游客登录拦截、登录后回跳、订单状态筛选、订单唯一性、订单详情选择、首页商品跳转一致性、支付结果测试态隐藏和关键防重叠布局约束；
- 2026-07-27 验收结果：8 项测试全部通过，JavaScript 与 JSON 静态检查错误为 0；
- 已在微信开发者工具实测首页、分类、我的、服务详情、确认订单和订单筛选页面；
- 服务详情双按钮点击态、顶部客服位置、分类卡片和确认订单底部滚动已完成视觉复测。
...
- 已完成 17 个原生微信小程序页面及四栏 TabBar；
- 已使用自有品牌占位素材，并完成首页、分类、我的、消息、详情、确认订单和订单列表第一轮交互；
- 已修复客服遮挡、底部双按钮重叠、分类卡片挤压、订单重复和登录绕过等问题；
- 当前仍使用本地演示数据和模拟支付，不视为内部可用业务系统。

### 阶段 B：内部可用版

- 本阶段为当前下一步；先完成服务端/后台底座选型和数据库设计；
...
- 当前顾客端继续采用原生微信小程序，不使用 ShopXO UniApp 重写；
- Tab 顺序为“首页、分类、我的、消息”；
- 四个主页面使用顶部人工客服，服务详情使用底部客服双按钮；
- 试运营首发暂定无畏契约，其余五个游戏待运营配置后开放。

### 21.2 开工前由运营补充的内容

- 最终产品品牌名、Logo 和正式横幅素材；当前“爆爆熊电竞”作为工作品牌；
- 无畏契约的正式平台、区服、营业时间和首发套餐；
- 六个游戏各专区的正式套餐、原价、现价、数量单位和交付标准；
- 服务团队说明、平台/区服和营业时间；
- 派单承诺时长、自动完成时限和退款规则；
- 客服账号、投诉流程和正式协议；
...
本 v2.1 文档替代《同类游戏服务小程序-PRD-v2.0-参考原型版》和《游戏陪玩护航小程序-PRD-v1.0》，作为后续任务拆分与实施验收基线。旧版仅用于追溯参考原型观察过程。
