# 同类游戏服务小程序 PRD（云开发实施版）

| 项目 | 内容 |
|---|---|
| 文档版本 | v2.1 |
| 文档状态 | 待云东评审；评审通过后作为唯一实施基线 |
| 编制日期 | 2026-07-27 |
| 参考原型 | 微信小程序“云玩电竞-陪玩开黑” |
| 观察方式 | 已在云东电脑上逐页查看实际小程序 |
| 实施方法参考 | 飞书《Codex App 从 0 到 1 开发一个小程序》中的“原型 → 云开发初始化 → Mock 盘点 → 数据库/API 设计 → 后端 → 后台”流程 |
| 目标产品 | 与参考原型结构、体验和视觉气质相近的自营游戏服务小程序 |
| 商业模式 | 单商户自营、后台人工派单 |
| 首版终端 | 微信小程序顾客端 + Web 运营后台 |
| 技术路线 | 原生微信小程序 + 微信云开发（CloudBase） |
| 云环境 ID | `cloud1-d5gmfvq70c644d633` |
| 小程序项目 | `D:\脚本\xcx\miniprogram-v2` |
| 当前阶段 | 阶段 A 原型基本完成，准备进入阶段 B 云端内部可用版 |
...
- 技术路线由 ShopXO + UniApp 调整为当前已经采用的原生微信小程序 + 微信云开发；
- 固定开发环境 ID，并补充云函数、云数据库、云存储和云静态网站边界；
- 底部导航顺序统一为“首页、分类、我的、消息”；
- 增加当前项目完成度，明确原型功能不等于真实业务功能；
- 参考飞书教程，将阶段 B 调整为“Mock 盘点 → 数据库/API 设计评审 → 云函数开发 → 后台开发 → 云端发布”；
- 新增阶段文档产物和退出条件，未经评审不得直接进入后端编码。
...
### 4.4 当前实现基线（2026-07-27）

已经完成：

- 17 个 Web 原型页面已转换为原生微信小程序页面；
- 首页、分类、我的、消息四栏原生 TabBar 与本地图标已完成；
- 主要页面跳转、本地演示订单和基础交互可运行；
- 微信云开发已在小程序启动时初始化，环境为 `cloud1-d5gmfvq70c644d633`；
- `cloudfunctions/` 已配置为云函数根目录；
- 已有自动化交互检查，但尚未覆盖全部真实云端业务。

尚未完成：

- 当前页面仍大量使用本地 Mock 数据和本地存储，尚未形成真实业务数据；
- 微信登录、云数据库、云函数、云存储和数据权限尚未接入；
- 微信支付、支付回调、退款和对账尚未接入；
- Web 运营后台、人工派单、角色权限和审计尚未开发；
- 正式套餐、价格、区服、履约规则、协议和素材尚未全部确定；
- 当前入口仍为登录页，与“先浏览、支付前登录”的产品原则不一致，阶段 B 首批改造时调整为首页入口。

本节用于说明“现在从哪里开始”，不得把原型中的演示数据视为已完成功能。
...
1. 顾客端继续使用当前原生微信小程序项目，不再采用 ShopXO、UniApp 或其他商城底座重新开发。
2. 后端统一使用微信云开发（CloudBase），固定开发环境 ID 为 `cloud1-d5gmfvq70c644d633`。
3. 云端能力使用云数据库、云函数、云存储和云静态网站；首版不引入自建服务器、微服务、多租户或复杂消息集群。
4. Web 运营后台使用独立前端项目，通过管理员专用云函数访问同一云数据库，完成后部署到云静态网站；独立域名不是内部版前提。
5. 小程序前端按 750rpx 设计，优先适配微信主流屏幕与底部安全区。
6. 首页、专区、商品和横幅全部由后台配置，不把参考文案和业务数据写死在前端。
7. 商品订单采用服务履约模型，不出现物流、收货地址或快递发货概念。
8. 支付状态、履约状态和售后状态独立保存，所有状态变化记录操作日志。
9. 登录、价格试算、订单写入、优惠券核销、派单、支付和退款必须通过云函数完成；顾客端不得直接修改受保护数据。
10. 微信登录使用云函数上下文取得可信 `OPENID`，客户端上传的用户 ID 不作为身份依据。
11. 微信支付使用 APIv3；AppSecret、商户号密钥、证书和 APIv3 密钥只保存在安全的云端配置中，不进入小程序代码、数据库普通字段或版本库。
12. 首版消息为系统通知，不建设顾客与服务人员私聊。
13. 人工派单由 Web 运营后台完成，服务人员首版不拥有公开端。
14. 评价与累计接单由真实订单聚合，后台只做违规内容审核，不允许修改星级或虚增数据。
15. 优惠券作为 P0；余额和邀请作为 P1 功能开关。
16. 代打采用全局开关 + 逐游戏开关，默认关闭。
17. 不提供密码字段；疑似密码、验证码和支付凭证输入应提示并阻止。
18. 正式视觉资产由经营主体提供或确认授权，开发阶段使用原创占位素材。
...
miniprogram-v2/
  ...
  admin-web/             Web 运营后台（阶段 B 后半段创建）
  docs/                  数据盘点、数据库、接口和验收文档
```

建议按业务域组织少量云函数，避免“一接口一个云函数”造成维护困难：

- `auth`：登录、用户初始化、用户资料；
- `catalog`：首页、游戏、专区、套餐、搜索；
- `order`：价格试算、创建订单、列表、详情、取消、确认；
- `payment`：预支付、支付通知、查单、关单、退款；
- `service`：派单、改派、履约状态；
- `customer`：优惠券、评价、投诉、消息；
- `admin`：后台内容、人员、权限、审计和配置。

具体函数动作、参数和返回值在 `docs/api.md` 中冻结后再实现。

### 12.2 云数据库边界

首版核心集合建议如下，具体字段、索引和权限在 `docs/database.md` 中定义：

| 业务域 | 集合 |
|---|---|
| 用户与权限 | `users`、`admin_users`、`roles` |
| 内容目录 | `games`、`categories`、`services`、`banners`、`recommendations` |
| 服务人员 | `service_staff` |
| 订单履约 | `orders`、`dispatch_records`、`order_logs` |
| 支付售后 | `payment_records`、`refund_records`、`complaints` |
| 营销口碑 | `coupon_templates`、`user_coupons`、`reviews` |
| 通知配置 | `messages`、`agreements`、`system_settings` |

设计约束：

- 金额使用整数分；
- 历史订单保存不可变商品、价格、参数和协议快照；
- 顾客只能读取自己的订单、券、消息和投诉；
- 运营后台所有写操作校验角色权限并写入审计日志；
- 支付、优惠券、派单和状态更新必须幂等；
- 测试数据须显式标记，不参与真实销量和评分统计。

### 12.3 环境与发布

- 当前开发环境：`cloud1-d5gmfvq70c644d633`；
- 开发、体验和正式发布必须区分配置，不允许把密钥写入前端；
- 云函数先在开发环境验证，再上传部署；
- Web 后台完成权限与安全验收后再部署到云静态网站；
- 正式上线前另建生产环境或明确现有环境的生产切换方案，并完成数据备份与回滚演练。
...
- 已完成首页、分类、详情、确认订单、消息、我的和订单列表等原生小程序页面；
- 已完成四栏 TabBar、页面跳转和主要本地交互；
- 退出条件：按本 PRD 第 16 节逐页复核，并把小程序默认入口从登录页调整为首页；
- 原型阶段允许使用演示数据，但必须在下一阶段逐项登记并替换。

### 阶段 B0：数据盘点与设计冻结（下一步）

本阶段参考“先找出假数据，再设计数据库和接口，评审后才写后端”的实施方法。

1. 扫描当前小程序全部页面、组件、工具文件和本地存储；
2. 列出每一处 Mock 数据、使用页面、字段、交互和预期真实来源；
3. 形成 `docs/mock-data-inventory.md`；
4. 根据本 PRD 设计云数据库集合、字段、索引、权限和测试数据，形成 `docs/database.md`；
5. 设计云函数动作、参数、身份权限、幂等和错误码，形成 `docs/api.md`；
6. 设计可重复导入的开发测试数据，形成 `docs/seed-data.md`；
7. 云东确认上述文档后，才进入云函数和真实接口开发。

退出条件：每个页面所需数据都能映射到明确集合和云函数，不存在“开发时再决定”的核心业务字段。

### 阶段 B1：云端基础与真实目录

- 初始化数据库集合、索引、权限规则和开发测试数据；
- 完成微信登录、自动注册、平台用户 ID 和注册成功消息；
- 首页、分类、搜索、套餐详情从云端加载，不再读取对应 Mock 数据；
- 后台暂可使用云开发控制台维护测试内容；
- 完成加载、空状态、失败重试、分页和下架过滤。

退出条件：访客可浏览真实云端目录，登录后可识别唯一用户，页面不依赖目录类 Mock 数据。

### 阶段 B2：订单、优惠券与履约

- 完成价格试算、优惠券校验、创建订单和订单快照；
- 完成订单列表、详情、取消、确认、异议和状态时间线；
- 完成人工派单、改派和履约状态更新；
- 完成系统消息、未读数、评价和投诉基础闭环；
- 所有受保护写操作通过云函数并记录日志。

退出条件：不接真实支付也能使用明确的开发支付状态完成整条内部履约演练，且开发状态入口不会出现在正式顾客界面。

### 阶段 B3：微信支付与售后

- 接入微信支付 APIv3 预支付；
- 完成支付通知验签、金额校验、幂等、查单和关单；
- 完成全额/部分退款、退款通知和对账记录；
- 以小额真实支付验证成功、取消、未知、重复通知和退款场景。

退出条件：只有可靠云端结果可以改变付款状态，重复请求不会重复扣券、记账、派单或退款。

### 阶段 B4：Web 运营后台

- 在 `admin-web/` 开发内容、专区、套餐、订单、人工派单、人员、优惠券、评价、投诉和设置；
- 完成管理员登录、角色权限、敏感字段脱敏和操作审计；
- 后台只通过管理员云函数访问业务数据；
- 内部验收后部署至 CloudBase 云静态网站；
- 生产使用前配置正式域名、访问控制和备份策略。

退出条件：运营无需修改数据库原始记录即可完成日常配置、派单和售后操作。

### 阶段 B5：内部可用版验收

- 按第 16、17 节完成业务、数据真实性、安全和视觉验收；
- 使用微信开发者工具、体验版和至少两台真机测试；
- 对照 `docs/mock-data-inventory.md` 确认 P0 Mock 数据全部替换；
- 形成 `docs/acceptance-checklist.md` 并记录未通过项。
...
- 服务包含陪玩、护航/代打；
- 已有经营主体、小程序和微信支付条件；
- 顾客端使用原生微信小程序，后端使用微信云开发；
- 当前云环境为 `cloud1-d5gmfvq70c644d633`；
- 底部导航顺序为：首页、分类、我的、消息；
- Web 运营后台部署到 CloudBase 云静态网站；
...
本 v2.1 文档替代《同类游戏服务小程序-PRD-v2.0-参考原型版》和《游戏陪玩护航小程序-PRD-v1.0》。旧版只能作为历史参考，产品范围、技术路线、实施顺序和验收标准均以本文件为准。
